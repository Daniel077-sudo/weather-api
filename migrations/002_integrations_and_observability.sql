...
alter table public.ai_suggestion_cache add column if not exists cache_key text;
alter table public.ai_suggestion_cache add column if not exists prompt_type text;
alter table public.ai_suggestion_cache add column if not exists subject text;
alter table public.ai_suggestion_cache add column if not exists context_hash text;
alter table public.ai_suggestion_cache add column if not exists response jsonb default '{}'::jsonb;
alter table public.ai_suggestion_cache add column if not exists created_at timestamptz default now();
create unique index if not exists ai_suggestion_cache_cache_key_idx on public.ai_suggestion_cache(cache_key);

alter table public.event_weather_alerts add column if not exists user_id text;
